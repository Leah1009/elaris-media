...
import { PromotionNotifyButton } from "@/components/promotion-notify-button";

export default async function PromotionsPage() {
  const ctx = await getBusinessContext();
  const supabase = await createClient();

  const [{ data: promotions }, { data: clients }] = await Promise.all([
    supabase
      .from("promotions")
              ...
              <tr key={p.id} className="border-b border-border last:border-0">
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center justify-end gap-3">
                    <PromotionNotifyButton promotion={p} businessName={ctx.business.name} clients={clients ?? []} />
