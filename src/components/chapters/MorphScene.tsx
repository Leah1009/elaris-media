"use client";

import { Canvas } from "@react-three/fiber";
import MorphRig from "./MorphRig";
import type { ChapterPart } from "@/lib/chapters/types";

type MorphSceneProps = {
  parts: ChapterPart[];
  progressRef: React.MutableRefObject<number>;
  pointerRef: React.MutableRefObject<{ x: number; y: number }>;
  lowDetail: boolean;
};

export default function MorphScene({ parts, progressRef, pointerRef, lowDetail }: MorphSceneProps) {
  return (
    <Canvas
      shadows={!lowDetail}
      dpr={lowDetail ? 1 : [1, 1.75]}
      camera={{ position: [0, 0, 5.2], fov: 32 }}
      gl={{ antialias: !lowDetail, powerPreference: "high-performance" }}
    >
      <color attach="background" args={["#F3E8D6"]} />
      <ambientLight intensity={0.65} />
      <directionalLight
        position={[3, 4, 5]}
        intensity={1.4}
        color="#FBF6EA"
        castShadow={!lowDetail}
      />
      <directionalLight position={[-4, 1, 3]} intensity={0.4} color="#FBF6EA" />
      <pointLight position={[-3, -2, -2]} intensity={0.35} color="#B08A54" />
      <MorphRig parts={parts} progressRef={progressRef} pointerRef={pointerRef} lowDetail={lowDetail} />
    </Canvas>
  );
}
